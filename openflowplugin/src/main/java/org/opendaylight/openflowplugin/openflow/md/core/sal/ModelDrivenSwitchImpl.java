...
import org.opendaylight.yang.gen.v1.urn.opendaylight.role.config.rev140626.UpdateControllerRoleInput;
import org.opendaylight.yang.gen.v1.urn.opendaylight.role.config.rev140626.UpdateControllerRoleOutput;
import org.opendaylight.yang.gen.v1.urn.opendaylight.role.config.rev140626.UpdateControllerRoleOutputBuilder;
...
public class ModelDrivenSwitchImpl extends AbstractModelDrivenSwitch {
    ...
    @Override
    public Future<RpcResult<UpdateControllerRoleOutput>> updateControllerRole(UpdateControllerRoleInput input) {
        UpdateControllerRoleOutput updateControllerRoleOutput = null;
        RoleRequestInputBuilder  builder = new RoleRequestInputBuilder();
        builder.setGenerationId(input.getGenerationId());
        builder.setRole(ControllerRole.forValue(input.getControllerRole().getIntValue()));
        builder.setVersion(version);
        Long xid = this.getSessionContext().getNextXid();
        builder.setXid(xid);
        Future<RpcResult<RoleRequestOutput>> roleOutputFuture =  messageService.roleRequest(builder.build(), null);
        try {
            RpcResult<RoleRequestOutput> rpcResult = roleOutputFuture.get(5, TimeUnit.SECONDS);
            if (rpcResult.isSuccessful()) {
                RoleRequestOutput roleRequestOutput = rpcResult.getResult();
                if (xid == roleRequestOutput.getXid().longValue()) {
                    LOG.error("Kamal MDSI-> xid matches :"+xid);
                    updateControllerRoleOutput = (new UpdateControllerRoleOutputBuilder())
                        .setControllerRole(ControllerRoleTypes.forValue(roleRequestOutput.getRole().getIntValue()))
                        .setNode(input.getNode())
                        .setGenerationId(roleRequestOutput.getGenerationId())
                        .setRpcStatus(true)
                        .build();
                    LOG.error("Kamal  MDSI-> updateControllerRoleOutput created");
                } else {
                    LOG.error("Future Xid={} for role change did not match the input xid={}", roleRequestOutput.getXid(), xid);
                    updateControllerRoleOutput = (new UpdateControllerRoleOutputBuilder()).setRpcStatus(false).build();
                }

            } else {
                LOG.error("Error in updateControllerRole RPC node={}, role={}, generationId={}", input.getNode(), input.getControllerRole(), input.getGenerationId());
                LOG.error("RpcErrors = "+ rpcResult.getErrors());
                updateControllerRoleOutput = (new UpdateControllerRoleOutputBuilder()).setRpcStatus(false).build();
            }

        } catch (Exception e) {
            LOG.error("Exception in roleChange: " + e.getMessage(), e);
            updateControllerRoleOutput = (new UpdateControllerRoleOutputBuilder()).setRpcStatus(false).build();
        }
        LOG.error("Kamal  MDSI-> creating rpcResult");
        SettableFuture<RpcResult<UpdateControllerRoleOutput>> rpcResult =  SettableFuture.create();
        LOG.error("Kamal  MDSI-> creating rpcResult : updateControllerRoleOutput="+updateControllerRoleOutput);
        rpcResult.set(Rpcs.getRpcResult(true, updateControllerRoleOutput, Collections.<RpcError> emptySet()));
        LOG.error("Returning rpcResult:" + rpcResult);
        return rpcResult;
    }
